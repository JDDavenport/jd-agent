...
export default function CryptoDetail() {
  ...
  const { data: markets } = useCryptoMarkets(id);
  ...
  if (!coin) {
    return (
      <EmptyState
        ...
        action={<Link to="/">Back to PoW Coins</Link>}
      />
    );
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center gap-3">
          <Button
            variant="secondary"
            onClick={() =>
              coin ? (watchlist.isWatching(coin.id) ? watchlist.remove(coin.id) : watchlist.add(coin.id)) : null
            }
          >
            {coin && watchlist.isWatching(coin.id) ? 'Remove from Watchlist' : 'Add to Watchlist'}
          </Button>
          {coin.websiteUrl && (
            <a
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <ScoreCard label="Censorship" value={coin.censorshipResistanceScore} accent="text-green-300" />
      </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="text-sm text-text-muted">
            {coin.networkHashRate
              ? `Hashrate ${formatHashrate(coin.networkHashRate, coin.networkHashRateUnit)}`
              : 'Hashrate —'}
      ...
      <Card>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          ...
          />
          <Metric label="Block Reward" value={coin.networkBlockReward ? coin.networkBlockReward.toString() : '—'} />
          <Metric
            label="ATH Date"
            value={coin.athDate ? new Date(coin.athDate).toLocaleDateString() : '—'}
          />
          <Metric label="Block Height" value={coin.networkBlockHeight ? formatCompact(coin.networkBlockHeight) : '—'} />
        </div>
      </Card>
      ...
      <Card>
        ...
        {markets && markets.length > 0 ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {markets.map((market, index) => (
                  <tr key={`${market.market}-${index}`} className="border-b border-dark-border">
                    <td className="px-4 py-2">{market.market}</td>
                    <td className="px-4 py-2">
                      {market.base}/{market.target}
                    </td>
                    <td className="px-4 py-2 text-right">{market.last ? formatCurrency(market.last) : '—'}</td>
                    <td className="px-4 py-2 text-right">{market.volume ? formatCompact(market.volume) : '—'}</td>
                    <td className="px-4 py-2">
                      {market.tradeUrl ? (
                        <a
          ...
          <p className="text-sm text-text-muted">No exchange listings available.</p>
        )}
      </Card>

      {peerComparison.length > 0 && (
        <Card>
          <h3 className="text-lg font-semibold mb-4">Algorithm Comparison ({coin.algorithm})</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="text-text-muted border-b border-dark-border">
                <tr>
                  ...
                  <tr key={peer.id} className="border-b border-dark-border">
                    ...
                    <td
                      className={`px-4 py-2 text-right ${
                        estimate?.profit && estimate.profit >= 0 ? 'text-green-400' : 'text-red-400'
                      }`}
                    >
                      {estimate ? formatCurrency(estimate.profit) : '—'}
                    </td>
                    <td className="px-4 py-2 text-right">
                      {estimate ? formatCurrency(estimate.revenue) : '—'}
                    </td>
                    <td className="px-4 py-2 text-right">
