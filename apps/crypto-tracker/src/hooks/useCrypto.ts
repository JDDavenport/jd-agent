...
export const cryptoKeys = {
  ...
  markets: (id?: string) => [...cryptoKeys.all, 'markets', id] as const,
};
...
export function useCryptoMarkets(id?: string) {
  return useQuery<CryptoMarketListing[]>({
    queryKey: cryptoKeys.markets(id),
...
export function useBugReportStatus() {
  return useQuery({
    queryKey: [...cryptoKeys.all, 'bug-report-status'],
    queryFn: getBugReportStatus,
    staleTime: 60 * 1000,
  });
}

export function useBugReportTest() {
  return useMutation({
    mutationFn: sendBugReportTest,
  });
}
