...
export class ApiClient {
  ...
  // Daily Journal
  async getDailyReview(date?: string): Promise<GetDailyReviewResponse> {
    const params = date ? `?date=${date}` : '';
    return this.request('GET', `/api/journal/daily-review${params}`);
  }

  async saveDailyReview(input: SaveReviewInput): Promise<DailyReview> {
    return this.request('POST', '/api/journal/daily-review/save', input);
  }

  async completeDailyReview(input: CompleteReviewInput): Promise<CompleteReviewResponse> {
    return this.request('POST', '/api/journal/daily-review/complete', input);
  }

  async getDailyReviewHistory(page?: number, limit?: number): Promise<ReviewHistoryResponse> {
    const params = new URLSearchParams();
    if (page) params.set('page', String(page));
    if (limit) params.set('limit', String(limit));
    const query = params.toString();
    return this.request('GET', `/api/journal/daily-review/history${query ? `?${query}` : ''}`);
  }

  async searchDailyReviews(query: string): Promise<ReviewHistoryItem[]> {
    return this.request('GET', `/api/journal/daily-review/search?q=${encodeURIComponent(query)}`);
  }

  async toggleHabitInReview(habitId: string, date?: string): Promise<HabitReviewData> {
    const params = date ? `?date=${date}` : '';
    return this.request('POST', `/api/journal/habits/${habitId}/toggle${params}`);
  }
